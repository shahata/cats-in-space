...
- **Iterate schemas individually, don't query all content at once**: Querying all EN content without a `schemaId` filter can miss entries — the general query doesn't return content from all schemas. Instead: (1) list all schemas via `GET /translation-schema/v1/schemas/site`, (2) for each schema, query EN and JA content separately with `schemaId` filter, (3) diff by `entityId` to find missing JA entries. This is the only reliable way to find the complete translation gap.
- **Per-schema queries work well with MCP**: Each schema has few entries, so per-schema query responses are small and won't hit MCP's ~54KB truncation limit. This makes the schema-by-schema approach the preferred method — no need for `WIX_API_KEY` + `curl`. Only fall back to API key + curl if you need to query all content across all schemas in a single sweep.
- **Some entries need `parentEntityId`**: Schemas with `requireParentEntity: true` (like Rich Text components) need the `parentEntityId` field copied from the EN entry when creating the JA entry. The MCP tool will miss entries beyond the truncation point.
