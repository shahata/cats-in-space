...
- **MCP truncates large responses at ~54KB**: The Wix MCP tool silently truncates responses larger than ~54KB. This means schema lists, content queries, and bulk operations may appear to return fewer results than actually exist. **Always use `WIX_API_KEY` with `curl` for translation work** — `curl -H "Authorization: $WIX_API_KEY" -H "wix-site-id: <siteId>"` — with proper cursor paging to see all results without truncation.
- **Use API key + cursor paging for full content**: To find the real translation gap, paginate through ALL EN and JA content via API key, build `schemaId::entityId` sets, and diff them. The MCP tool will miss entries beyond the truncation point.
