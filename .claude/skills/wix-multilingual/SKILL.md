...
- **MCP and API key show different schemas**: The Wix MCP tool and direct API key auth (`WIX_API_KEY` + `wix-site-id` header) may show different schemas and content. The API key auth reveals more schemas (e.g., 66 vs 46) and is the authoritative view. **Always use the API key for translation work** — `curl` with `-H "Authorization: $WIX_API_KEY" -H "wix-site-id: <siteId>"`. The MCP tool uses site-level auth that doesn't see all schemas.
- **Use API key to find the real gap**: Query EN and JA content via API key with cursor paging to get the true count of missing translations. The MCP tool truncates large responses and may miss entries.
- **Duplicate schemas**: Some content types have TWO schemas (e.g., Products has both `b8f0a427` visible via MCP and `7da0388d` visible via API key). Make sure to translate content for ALL schemas, not just the ones visible through one auth method.
