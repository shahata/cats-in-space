...
- [ ] **No separate Tickets tab — tickets surface in Orders.** `ticketReservations.createTicketReservation` → checkout flows through eCom, so event tickets land in `ecomOrders.searchOrders` like everything else. Classify by `catalogReference.appId` and badge as `ticket` (see `src/utils/appIds.ts`). Adding a separate Tickets tab duplicates orders for no reason.
