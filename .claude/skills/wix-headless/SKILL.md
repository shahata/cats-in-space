...
- [ ] **Orders tab (conditional — only if site has a store, donations, gift cards, any eCom purchases, OR event tickets)** — order history with line item images (via `getImageUrl()`), quantities, prices, status badges (color-coded), totals. Server-rendered. Omit only if NO purchase-backed features exist. Other feature-specific tabs follow the same pattern: Bookings tab if site has bookings, Subscriptions tab if site has pricing plans, etc.
  ...
  - **Event tickets DO NOT live in `ecomOrders.searchOrders`** — `redirects.createRedirectSession({ eventsCheckout })` creates orders in the separate Events system (`@wix/events` → `orders.listOrders`). If the site has a `/tickets` page, the Orders tab MUST also fetch `eventsOrders.listOrders({ memberId: [member._id] })`, look up event titles via parallel `wixEventsV2.getEvent(id)` calls, convert each event order to the same card shape (synthetic single-line-item with the event title + `'ticket'` badge), merge with ecom cards and sort by date desc. Without this second fetch, every ticket purchase is invisible to the member. See [MEMBER_AREA.md](references/MEMBER_AREA.md) → "Wix Events tickets are NOT in ecomOrders.searchOrders".
