...
export default function DonateCard({
  ...
  const [selectedId, setSelectedId] = useState<string>(
    presets[0]?._id ?? (customEnabled ? CUSTOM_ID : ""),
  );
  const [customAmount, setCustomAmount] = useState("");
  const defaultFreq: DonationFrequency = frequencies.includes(
    ...
    <div className="donate-card">
      ...
      <div className="donate-amount-grid">
        {presets.map((p) => {
          ...
          return (
            ...
            >
              ...
              {p.description && (
                <span className="donate-amount-desc">{p.description}</span>
              )}
            </button>
          );
        })}
        {customEnabled && (
          ...
          >
            <span className="donate-amount-price">
              {t("research.customAmount")}
            </span>
          </button>
        )}
      </div>
        ...
        <div className="donate-custom-wrap">
          ...
          {customMinFormatted && customMaxFormatted && (
            <span className="donate-custom-hint">
              {t("research.amountHint", {
                min: customMinFormatted,
                max: customMaxFormatted,
              })}
            </span>
          )}
        </div>
