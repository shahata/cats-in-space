...
- **Recurring events translate per occurrence**: each occurrence in a recurring series has its own `Event._id` and its own translation-content row. Iterate `queryEvents({ paging: { limit, offset } })` and create an entry per event id — translations don't propagate across siblings.
