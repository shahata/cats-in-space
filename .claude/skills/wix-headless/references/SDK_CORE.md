...
| **REST returns `id`, SDK returns `_id`** — applies to **every** V3 endpoint (products, customizations, info sections, categories, services, plans, posts, campaigns, …) | Seed scripts and any `httpClient.fetchWithAuth` / `wixFetch` consumer read `entity.id`; SDK callers read `entity._id`. Reading `res.product._id` from a REST response returns `undefined`, gets persisted to `state.json`, then breaks the next step with `404 Entity not found`. Use the `idOf(entity)` helper exported from `seed/lib/wix.mjs` (or write `entity.id ?? entity._id`) on every REST response. This is the **#1 first-run seed failure** |
| **SDK method names drop the `file` segment in REST paths** for the Media upload endpoint | SDK method `files.generateFileUploadUrl()` → REST path `POST /site-media/v1/files/generate-upload-url` (NOT `generate-file-upload-url`). The mistaken path returns a Wix-edge 404 HTML page (not a JSON error), which is easy to misread as "the API doesn't exist." Same naming drift may apply to other media endpoints; if a REST call to a `generate-file-*` path returns 404 HTML, try removing the `file` segment |
...
| `createMemberAbout(...)` and `updateMemberAbout(id, ...)` return `MemberAbout` **directly** (NOT wrapped). Only the `getMyMemberAbout()` *read* is wrapped | Asymmetric SDK — destructure on read, use directly on write: `const created = await membersAbout.createMemberAbout({...}); created._id; created.revision;` |
| `members.Address.addressLine` is **line 1**. There is no `addressLine1` field. Line 2 is `addressLine2` | The members SDK uses `addressLine` (no `1`) — `addressLine1` is a US-postal-style guess that astro check rejects with `Property 'addressLine1' does not exist. Did you mean 'addressLine'?` |
| `members.Address` / `members.Contact` fields are `string \| null`, not `string \| undefined`. Under `exactOptionalPropertyTypes` you must pass `null` (or omit the key) — passing `undefined` is a type error | `Type 'string \| undefined' is not assignable to type 'string \| null'`. Use `value || null` (not `value \|\| undefined`) when building update payloads |
| Cart estimate response type is `cart.EstimateTotalsResponse` (note: `cartTypes` namespace), **not** `currentCart.EstimateCurrentCartTotalsResponse` | The function lives on the `currentCart` module but its response type is exported from the parent `cart` namespace under the generic name |
| `productsV3.RequestedFields` is a real exported enum (use `productsV3.RequestedFields.MEDIA_ITEMS_INFO`, etc.) rather than string literals | Both work at runtime, but the enum gives IDE autocomplete and survives renames. Same pattern for `seoTagsApi.ItemType`, `currentCart.ChannelType`, `members.Set`, etc. |
| `suppressHydrationWarning` is **React-only**. Putting it on an Astro `<button>` / `<input>` template element fails astro check with `Property 'suppressHydrationWarning' does not exist on type 'ButtonHTMLAttributes'` | This attribute is a React-specific escape hatch for hydration mismatches inside `client:load` islands. Astro template elements never hydrate — they're either SSR-only or replaced by an island, so the attribute is meaningless there. Only add it to JSX elements *inside* `.tsx` files |
...
export const GIFT_CARDS_APP_ID = 'd80111c5-a0f4-47a8-b63a-65b54d774a27';
