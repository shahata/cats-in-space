...
export const POST: APIRoute = async ({ request }) => {
	try {
		...
		if (!operationId || !/^\d{4}-\d{2}-\d{2}$/.test(date)) {
			return new Response(JSON.stringify({ error: 'Invalid operationId or date' }), { status: 400 });
		}
		const [y, m, d] = date.split('-').map(Number);
		const elevated = auth.elevate(operationsApi.calculateAvailableTimeSlotsForDate);
		...
		const deliveryServiceable = !address || (slotsByType.DELIVERY?.length ?? 0) > 0;
