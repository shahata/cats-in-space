...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!file || !memberId) {
      return new Response(
        JSON.stringify({ error: "file and memberId required" }),
        { status: 400 },
      );
    }
    ...
    if (!dimensions.width || !dimensions.height) {
      return new Response(
        JSON.stringify({ error: "Could not read image dimensions." }),
        { status: 400 },
      );
    }
    ...
    const result = await elevatedGenerateUrl(file.type, {
      fileName: file.name,
    });
    ...
    if (!uploadRes.ok) {
      return new Response(
        JSON.stringify({ error: "Upload to media manager failed" }),
        { status: 500 },
      );
    }
    ...
    if (!photoId || !photoUrl) {
      return new Response(
        JSON.stringify({ error: "Upload succeeded but no file data returned" }),
        { status: 500 },
      );
    }
...
export const DELETE: APIRoute = async ({ request }) => {
  try {
    ...
    const profileUpdate =
      field === "cover" ? { cover: { url: "" } } : { photo: { url: "" } };
    await members.updateMember(memberId, { profile: profileUpdate });
