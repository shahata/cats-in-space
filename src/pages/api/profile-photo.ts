...
export const POST: APIRoute = async ({ request }) => {
	try {
		...
		if (dimensions.width < MIN_DIMENSION || dimensions.height < MIN_DIMENSION) {
			return new Response(JSON.stringify({ error: `Image too small. Minimum ${MIN_DIMENSION}x${MIN_DIMENSION}px.` }), {
				status: 400,
			});
		}
		if (dimensions.width > MAX_DIMENSION || dimensions.height > MAX_DIMENSION) {
			return new Response(JSON.stringify({ error: `Image too large. Maximum ${MAX_DIMENSION}x${MAX_DIMENSION}px.` }), {
				status: 400,
			});
		}
		...
		const profileUpdate =
			field === 'cover' ? { cover: { _id: photoId, url: photoUrl } } : { photo: { _id: photoId, url: photoUrl } };
