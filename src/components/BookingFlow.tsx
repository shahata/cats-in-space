...
export default function BookingFlow({ serviceId, serviceName, duration, staff }: BookingFlowProps) {
	...
	async function findNextAvailable() {
		...
		try {
			...
			const result = await availabilityCalendar.queryAvailability({ filter }, { timezone: tz, slotsPerDay: 1 });
			...
			if (entries.length > 0 && entries[0]?.slot?.startDate) {
				...
				const dayResult = await availabilityCalendar.queryAvailability({ filter: dayFilter }, { timezone: tz });
		...
		<div style={styles.container}>
			...
			{step === 'time' && (
				<div>
					<p style={styles.stepLabel}>
						{selectedDate &&
							new Date(selectedDate + 'T12:00:00').toLocaleDateString(i18n.getLocale(), {
								weekday: 'long',
								month: 'long',
								day: 'numeric',
							})}
					...
					{loading ? (
						...
						<div style={styles.slotsGrid}>
							{availabilityEntries.map((entry, i) => (
								<button key={i} onClick={() => handleSelectSlot(entry)} style={styles.slotBtn}>
