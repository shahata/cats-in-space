...
export default function BookingFlow({
  ...
  const [availabilityEntries, setAvailabilityEntries] = useState<
    availabilityTypes.SlotAvailability[]
  >([]);
  const [selectedEntry, setSelectedEntry] =
    useState<availabilityTypes.SlotAvailability | null>(null);
  const [loading, setLoading] = useState(false);
  const [searching, setSearching] = useState(false);
  const [booking, setBooking] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [step, setStep] = useState<"staff" | "date" | "time" | "confirm">(
    "staff",
  );
  ...
  async function fetchAvailability() {
    ...
    try {
      ...
      const entries = (result.availabilityEntries ?? []).filter(
        (e) => e.bookable,
      );
    ...
    try {
      ...
      if (entries.length > 0 && entries[0]?.slot?.startDate) {
        ...
        const dayFilter: Record<string, unknown> = {
          ...
          endDate: new Date(
            new Date(dateStr + "T00:00:00").getTime() + 86400000,
          ).toISOString(),
        };
        ...
        setAvailabilityEntries(
          (dayResult.availabilityEntries ?? []).filter((e) => e.bookable),
        );
      } else {
    ...
    <div style={styles.container}>
      ...
      <div style={styles.progress}>
        {(["staff", "date", "time", "confirm"] as const).map((stepName, i) => {
          const currentIdx = (
            ["staff", "date", "time", "confirm"] as const
          ).indexOf(step);
        ...
        <div>
          ...
          <div style={styles.staffGrid}>
            ...
            >
              ...
              <span style={styles.staffBtnName}>
                {t("bookings.anyAvailable")}
              </span>
            </button>
          </div>
        </div>
      ...
      {step === "date" && (
        <div>
          <p style={styles.stepLabel}>
            {selectedStaffInfo
              ? t("bookings.bookingWith", { name: selectedStaffInfo.name })
              : t("bookings.anyAvailableStaff")}
            {staff.length > 1 && (
              <button onClick={() => setStep("staff")} style={styles.changeBtn}>
                {t("bookings.change")}
              </button>
            )}
          </p>
          ...
          {loading ? (
            ...
            <div style={styles.noSlots}>
              ...
              >
                ...
                >
                  {searching
                    ? t("bookings.searching")
                    : t("bookings.findAvailableDate")}
                </button>
              </div>
            </div>
          ) : (
          ...
          <div style={styles.confirmCard}>
            ...
            <div style={styles.confirmRow}>
              ...
              <span style={styles.confirmValue}>
                {selectedStaffInfo
                  ? `${selectedStaffInfo.emoji} ${selectedStaffInfo.name}`
                  : t("bookings.anyAvailable")}
              </span>
            </div>
            <div style={styles.confirmRow}>
            ...
            <div style={styles.confirmRow}>
              ...
              <span style={styles.confirmValue}>
                {formatTime(selectedEntry.slot?.startDate)} —{" "}
                {formatTime(selectedEntry.slot?.endDate)}
              </span>
            </div>
            <div style={styles.confirmRow}>
              <span style={styles.confirmLabel}>{t("bookings.duration")}</span>
              <span style={styles.confirmValue}>
                {duration} {t("bookings.minutes")}
              </span>
            </div>
            <div style={styles.confirmRow}>
