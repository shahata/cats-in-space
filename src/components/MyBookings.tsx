  ...
  if (items.length === 0) {
    return (
      <div style={styles.empty}>
        ...
        <a href="/bookings" style={styles.bookLink}>
          {t("bookings.visitMedicalBay")}
        </a>
      </div>
    );
  }
      ...
      {upcoming.length > 0 && (
        <div>
          <h3 style={styles.sectionTitle}>
            {t("bookings.upcomingAppointments")}
          </h3>
          {upcoming.map((b) => {
            const d = getSlotDate(b);
                  ...
                  <div>
                    ...
                    {getStaffName(b) && (
                      <div style={styles.staffName}>
                        {t("bookings.with")} {getStaffName(b)}
                      </div>
                    )}
                  </div>
                  <span
                ...
                {(canCancel || canReschedule) && (
                  <div style={styles.cardActions}>
                    ...
                    {canCancel && b.booking?._id && b.booking?.revision && (
                      <button
                        onClick={() =>
                          handleCancel(b.booking!._id!, b.booking!.revision!)
                        }
                        disabled={actionLoading === b.booking?._id}
                        style={styles.cancelBtn}
                      >
                        {actionLoading === b.booking?._id
