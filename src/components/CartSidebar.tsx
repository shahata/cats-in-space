...
export default function CartSidebar() {
	...
	const fetchCart = useCallback(async () => {
		try {
			...
			setCart(c);
			const count = (c?.lineItems || []).reduce((sum, li) => sum + (li.quantity || 0), 0);
			setItemCount(count);
	...
	const subtotal = totals?.priceSummary?.subtotal?.formattedAmount || totals?.priceSummary?.subtotal?.amount;
	const total = totals?.priceSummary?.total?.formattedAmount || totals?.priceSummary?.total?.amount;
				...
				{lineItems.length === 0 ? (
					...
					<>
						<div className="cs-items">
							{lineItems.map((li) => {
								const rawImg =
									typeof li.image === 'string' ? li.image : (li.image as { url?: string } | undefined)?.url;
								...
								const modifierGroups = (
									...
									.map((g) => {
										const groupName = g.name?.translated || g.name?.original || '';
										const rows = (g.modifiers || [])
											.map((m) => {
												const label = m.label?.translated || m.label?.original || '';
												if (!label) return null;
												return {
													qty: m.quantity ?? 1,
													label,
													price: m.price?.formattedAmount || m.price?.amount || '',
												};
											})
											.filter((r): r is { qty: number; label: string; price: string } => r !== null);
								...
								const isRestaurantLine = li.catalogReference?.appId === RESTAURANTS_APP_ID;
								const canEditInModal =
									isRestaurantLine &&
									typeof window !== 'undefined' &&
									window.location.pathname.includes('/restaurant/order');
								const clickHandler = canEditInModal
									? () => editRestaurantLine(li._id!, li.catalogReference!.catalogItemId!)
									: undefined;
								...
								return (
									...
									>
										...
										<div className="cs-item-details">
												...
												<div className="cs-item-mods">
													{modifierGroups.map((g, gi) => (
														<div key={gi} className="cs-mod-group">
															{g.name && <div className="cs-mod-group-name">{g.name}:</div>}
															{g.rows.map((r, ri) => (
											...
											<div className="cs-item-price">{li.price?.formattedAmount || li.price?.amount}</div>
											<div className="cs-item-qty" onClick={(e) => e.stopPropagation()}>
												<button
						...
						<div className="cs-footer">
							...
							<button className="cs-checkout-btn" onClick={checkout} disabled={checkingOut || loading}>
