...
export default function GiftCardActions({ productId, productImage, presetVariants, customVariant }: Props) {
	...
	const selectedPreset = presetVariants.find((v) => v._id === selectedVariantId);
	const parsedCustom = parseFloat(customAmount);
	const canBuy = selectedVariantId && (!isCustom || parsedCustom > 0) && !loading;

	// Current image: variant image > product image
	const currentImage = selectedPreset?.image || productImage || null;

	// Price vs value display
	const priceAmount = selectedPreset ? parseFloat(selectedPreset.price.amount) : 0;
	const valueAmount = selectedPreset ? parseFloat(selectedPreset.value.amount) : 0;
		...
		<div className="gc-actions">
			...
			{selectedPreset && (
				<div className="gc-price-display">
					{hasDiscount && <span className="gc-price-original">{selectedPreset.value.formattedAmount}</span>}
					<span className="gc-price-cost">{selectedPreset.price.formattedAmount}</span>
				</div>
			)}
					...
					<div className="gc-custom-input-wrap">
						...
						{customVariant.minValue && customVariant.maxValue && (
							<span className="gc-range-hint">
								{customVariant.minValue.formattedAmount} – {customVariant.maxValue.formattedAmount}
							</span>
						)}
					</div>
				)}
			...
			<button className="gc-purchase-btn" disabled={!canBuy} onClick={handleBuyNow}>
