...
export default function GiftCardActions({
  ...
  const currentImage =
    selectedPreset?.image || productImage || null;
    ...
    <div className="gc-actions">
      ...
      {selectedPreset && (
        <div className="gc-price-display">
          {hasDiscount && (
            <span className="gc-price-original">{selectedPreset.value.formattedAmount}</span>
          )}
        ...
        <div className="gc-amount-options">
          {presetVariants.map((v) => (
            <button
              ...
              onClick={() => { setSelectedVariantId(v._id); setError(null); }}
            >
          ...
          {customVariant && (
            <button
              ...
              onClick={() => { setSelectedVariantId(CUSTOM_VARIANT_ID); setError(null); }}
            >
      ...
      {error && (
        <div className="gc-status gc-status-error">{error}</div>
      )}
    </div>
  );
}
