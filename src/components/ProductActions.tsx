...
export default function ProductActions({ product }: Props) {
	...
	const selectedVariant = useMemo(() => {
		...
		return variants.find((v) => {
			...
			return Object.entries(selections).every(([optName, choiceVal]) =>
				choices.some(
					(c) => c.optionChoiceNames?.optionName === optName && c.optionChoiceNames?.choiceName === choiceVal,
				),
			);
		});
	...
	const missingRequired = freeTextModifiers.filter((m) => m.mandatory).some((m) => !customTexts[m.name!]?.trim());
	...
	const submitBackInStock = async () => {
		...
		try {
			...
			setMessage({
				type: 'error',
				text: e instanceof Error ? e.message : t('product.failedNotification'),
			});
		} finally {
			setLoading(null);
		}
		...
		<div className="pa-root">
			...
			{isInStock ? (
				<>
					<div className="pa-quantity">
						...
						<div className="pa-qty-controls">
							...
							<button className="pa-qty-btn" onClick={() => setQuantity(quantity + 1)}>
								+
							</button>
						</div>
					</div>
					...
					<div className="pa-actions">
						<button className="pa-btn pa-btn-cart" onClick={addToCart} disabled={loading !== null}>
							{loading === 'cart' ? t('product.adding') : isPreOrder ? t('product.preOrder') : t('product.addToCart')}
						</button>
						<button className="pa-btn pa-btn-buy" onClick={buyNow} disabled={loading !== null}>
				...
				<div className="pa-bis">
					{!bisSubmitted ? (
						<>
							<p className="pa-bis-text">{t('product.backInStockText')}</p>
			...
			{message && <div className={`pa-message pa-message-${message.type}`}>{message.text}</div>}
		</div>
	);
}
