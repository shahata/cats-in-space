...
export default function BlogEngagement({
	...
	const [memberProfiles, setMemberProfiles] = useState<
		Map<string, { nickname: string; title?: string; photo?: string; slug?: string }>
	>(new Map());
	...
	async function reportView() {
		try {
			await httpClient.fetchWithAuth(`https://www.wixapis.com/blog/v3/posts/${postId}/view`, { method: 'POST' });
		} catch {}
	}
				...
				nodes: [
					{
						...
						nodes: [{ type: 'TEXT' as const, textData: { text, decorations: [] } }],
						paragraphData: {},
					},
				],
			},
			...
			>
					...
					<div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
						{rAuthorInfo.profileUrl ? (
							<a href={rAuthorInfo.profileUrl} style={authorLinkStyle}>
								{rAuthorInfo.photo && (
									<img src={rAuthorInfo.photo} alt={rAuthor} style={avatarStyle} referrerPolicy="no-referrer" />
								)}
				...
				{!rIsEditing && (
					<div style={commentActionsStyle}>
						...
						>
							{likedCommentIds.has(replyId) ? t('blog.likedComment') : t('blog.likeComment')}
						</button>
						...
						{rIsMine && (
							<>
								...
								<button onClick={() => handleDelete(replyId)} style={{ ...actionBtnStyle, color: '#cc0000' }}>
									{t('blog.delete')}
								</button>
							</>
						)}
					...
					>
						{isLoggedIn ? (
							<div style={{ ...memberIndicatorStyle, marginBottom: '8px' }}>
								{memberPhoto && (
									<img src={memberPhoto} alt={memberName} style={avatarStyle} referrerPolicy="no-referrer" />
								)}
								<span>
									{t('blog.replyingAs')} <strong style={{ color: '#ffcc00' }}>{memberName}</strong>
								</span>
							</div>
						) : (
							<input
				...
				{nestedReplies.length > 0 && (
					<div style={{ marginTop: '8px' }}>{nestedReplies.map((nr) => renderReplyThread(nr, depth + 1))}</div>
				)}
			</div>
		);
	}
			...
			<div key={commentId + '-' + myVisitorId} style={commentCardStyle}>
					...
					<div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
						{authorInfo.profileUrl ? (
							<a href={authorInfo.profileUrl} style={authorLinkStyle}>
								{authorInfo.photo && (
									<img src={authorInfo.photo} alt={authorInfo.name} style={avatarStyle} referrerPolicy="no-referrer" />
								)}
				...
				{!isEditing && (
					<div style={commentActionsStyle}>
						...
						>
							{likedCommentIds.has(commentId) ? t('blog.likedComment') : t('blog.likeComment')}
						</button>
						{commentingEnabled && !loginRequired && (
							<button
								onClick={() => {
						...
						{isMine && (
							<>
								...
								<button onClick={() => handleDelete(commentId)} style={{ ...actionBtnStyle, color: '#cc0000' }}>
									{t('blog.delete')}
								</button>
							</>
						)}
				...
				{replies.length > 0 && (
					<div style={{ marginTop: '12px' }}>{replies.map((reply) => renderReplyThread(reply, 1))}</div>
				)}
			</div>
		);
	});
					...
					<form onSubmit={submitComment} style={commentFormStyle}>
						...
						{isLoggedIn ? (
							<div style={memberIndicatorStyle}>
								...
								<span>
									{t('blog.commentingAs')} <strong style={{ color: '#ffcc00' }}>{memberName}</strong>
								</span>
							</div>
						) : (
							<input
