...
function getCardDisplay(pm: SavedPaymentMethod) {
	...
	const brandLabel = brandIcons[brand]
		? `\uD83D\uDCB3 ${brandIcons[brand]}`
		: typeId === 'payPal'
			? '\uD83C\uDD7F\uFE0F PayPal'
			: `\uD83D\uDCB3 ${brand}`;
...
export default function SavedPaymentMethodsPanel({ initial }: { initial: SavedPaymentMethod[] }) {
	...
	if (methods.length === 0) {
		return (
			<div style={styles.empty}>
				...
				<p style={styles.emptyHint}>{t('payment.savedDuringCheckout')}</p>
			</div>
		);
	}
						...
						<div key={pm._id} style={styles.card}>
							<div style={styles.cardMain}>
								...
								<div style={styles.cardMeta}>
									...
									{exp && (
										<span style={styles.exp}>
											{t('payment.expPrefix')} {exp}
										</span>
									)}
								</div>
							</div>
							<div style={styles.actions}>
								{!pm.primary && (
									<button onClick={() => handleSetPrimary(pm._id!)} disabled={isLoading} style={styles.btnSecondary}>
										{isLoading ? '...' : t('payment.setPrimary')}
									</button>
								)}
								<button onClick={() => handleDelete(pm._id!)} disabled={isLoading} style={styles.btnDanger}>
