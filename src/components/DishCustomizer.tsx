...
export default function DishCustomizer({ item }: Props) {
  ...
  const [selectedVariant, setSelectedVariant] = useState<string>(
    hasVariants ? item.priceVariants[0]?._id || "" : ""
  );
  ...
  const currency = hasVariants
    ? item.priceVariants[0]?.price.currency || "USD"
        ...
        lineItems: [
          {
            ...
            catalogReference: {
              ...
              options: {
                ...
                ...Object.fromEntries(
                  Object.entries(modifierSelections).filter(
                    ([, v]) => v.length > 0
                  )
                ),
              },
              ...
              {group.modifiers.map((mod) => {
                ...
                return (
                  <button
                    key={mod._id}
                    onClick={() =>
                      toggleModifier(group._id, mod._id, group.maxChoices)
                    }
                    style={{
                      ...styles.modifierBtn,
                    ...
                    {mod.additionalPrice &&
                      parseFloat(mod.additionalPrice.amount) > 0 && (
                        <span style={styles.modifierPrice}>
                          +{formatPrice(parseFloat(mod.additionalPrice.amount))}
                        </span>
                      )}
                  </button>
                );
