...
function extractCartLine(lineId: string, quantity: number, rawOptions: RawOptions, item: MenuItem): CartLine {
	...
	for (const g of rawOptions.modifierGroups || []) {
		...
		if (!groupId) continue;
		const modIds = (g.modifiers || []).map((m) => m.id ?? m._id ?? m.modifierId ?? '').filter(Boolean);
	...
	for (const group of item.modifierGroups) {
		...
		const names = selIds.map((id) => group.modifiers.find((m) => m._id === id)?.name).filter((n): n is string => !!n);
...
export default function MenuOrderView({
	...
	const applyDispatchToCart = async (type: string, slot: 'ASAP' | string, slotObj?: TimeSlotOption) => {
		try {
			...
			if (slot === 'ASAP') {
				...
				const resolved =
					slotObj ?? currentTypeSlots.find((s) => s.start === slot) ?? draftSlots.find((s) => s.start === slot);
	...
	const hasCustomization = (item: MenuItem): boolean => item.priceVariants.length > 0 || item.modifierGroups.length > 0;
	...
	const linePriceLabel = (item: MenuItem, line: CartLine): string => {
		const variant = line.variantId ? item.priceVariants.find((v) => v._id === line.variantId) : undefined;
		const base = variant ? parseFloat(variant.price.amount) : item.price ? parseFloat(item.price.amount) : 0;
		let extra = 0;
		for (const group of item.modifierGroups) {
	...
	const totalQtyFor = (itemId: string) => (cartLinesByItem[itemId] || []).reduce((s, l) => s + l.quantity, 0);
											...
											<>
												...
												<div className="mov-schedule-grid">
													...
													<label className="mov-schedule-field">
														...
														>
															{draftLoading && <option value="">{t('restaurant.loading')}</option>}
						...
						<div className="mov-grid">
							{section.items
								...
								.map(({ item, line, badge }) => {
									...
									return (
										<button key={key} type="button" className="mov-card" onClick={() => openItem(item, line)}>
											<div className="mov-card-body">
												...
												{line && line.summary.length > 0 && (
													<div className="mov-card-summary">
														<div className="mov-card-summary-line">{line.summary.join(' • ')}</div>
													</div>
												)}
											</div>
			...
			{modalItem && (
				<div className="mov-modal-overlay" onClick={closeModal}>
					<div className="mov-modal" onClick={(e) => e.stopPropagation()}>
						...
						<button type="button" className="mov-modal-close" onClick={closeModal} aria-label={t('common.close')}>
							×
						</button>
														...
														>
															...
															{mod.additionalPrice && parseFloat(mod.additionalPrice.amount) > 0 && (
																<span className="mov-mod-price">
																	+{formatAmount(parseFloat(mod.additionalPrice.amount))}
																</span>
															)}
														</button>
													);
									...
									<div className="mov-group">
										...
										<div className="mov-qty-row">
											<button
												...
												onClick={() =>
													setQty((p) => ({ ...p, [modalItem._id]: Math.max(1, (p[modalItem._id] || 1) - 1) }))
												}
											>
												−
											</button>
											...
											<button
												type="button"
												className="mov-qty-btn"
												onClick={() => setQty((p) => ({ ...p, [modalItem._id]: (p[modalItem._id] || 1) + 1 }))}
											>
												+
											</button>
										</div>
