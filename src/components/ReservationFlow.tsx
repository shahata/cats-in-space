...
type TimePeriod = NonNullable<
  NonNullable<
    NonNullable<ReservationLocation["configuration"]>["onlineReservations"]
  >["businessSchedule"]
...
export default function ReservationFlow({
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      setError(
        e instanceof Error ? e.message : t("restaurant.reservationFailed"),
      );
    } finally {
      setLoading(false);
    }
      ...
      <div style={styles.container}>
        <div style={styles.successBox}>
          ...
          <div style={styles.summaryCard}>
            <SummaryRow
              label={t("restaurant.reserveDate")}
              value={chosenSlot ? formatDate(selectedDate) : ""}
            />
            <SummaryRow
            ...
            />
            <SummaryRow
              label={t("restaurant.reserveParty")}
              value={`${partySize} ${t("restaurant.guests")}`}
            />
      ...
      {step === "search" && (
        <div>
          ...
          <div style={{ marginTop: 20 }}>
            <p
              style={{ ...styles.stepLabel, fontWeight: 600, marginBottom: 8 }}
            >
              {loading
            ...
            {!loading && availableSlots.length === 0 ? (
              ...
              <div style={{ ...styles.slotsGrid, opacity: loading ? 0.5 : 1 }}>
                {availableSlots.map((slot) => {
                  ...
                  const isSelected = chosenSlot
                    ? slotKey(chosenSlot) === key
                    : false;
                  return (
                    <button
                      key={key}
      ...
      {step === "details" && (
        <div>
          <p style={styles.stepLabel}>
            ...
            {chosenSlot ? formatSlotTime(chosenSlot.startDate) : ""} &middot;{" "}
            {partySize} {t("restaurant.guests")}
          </p>
          <p style={{ ...styles.stepLabel, fontWeight: 600 }}>
            {t("restaurant.guestDetails")}
          </p>
      ...
      {step === "confirm" && (
        <div>
          ...
          <div style={styles.summaryCard}>
            ...
            {guestPhone && (
              <SummaryRow
                label={t("restaurant.guestPhone")}
                value={guestPhone}
              />
            )}
            {specialRequests && (
              <SummaryRow
                label={t("restaurant.specialRequests")}
                value={specialRequests}
              />
            )}
          </div>
