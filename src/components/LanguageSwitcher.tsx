...
export default function LanguageSwitcher({ variant }: LanguageSwitcherProps) {
	...
	const currentFlag = langToFlag(currentLang?.regionalFormat) || currentLanguage.toUpperCase();
	const isRtl = ['he', 'ar'].includes(currentLanguage);

	if (variant === 'dropdown') {
		return (
			<div ref={ref} style={styles.trigger} onMouseEnter={() => setOpen(true)} onMouseLeave={() => setOpen(false)}>
				...
				{open && (
					<div style={isRtl ? styles.submenuRtl : styles.submenu}>
						{languages.map((lang) => (
							...
							>
								<span style={styles.flag}>{langToFlag(lang.regionalFormat) || (lang.id || '').toUpperCase()}</span>
